...
def _session(db, project_id: uuid.UUID, payload: bytes, *, chunk: int = 16, checksum: bool = True):
...
class TestTheHappyPath:
    def test_chunks_assemble_into_the_original_file(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
    ...
    def test_the_parts_are_cleaned_up_afterwards(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
        for index, chunk in enumerate(_chunks(PAYLOAD, session.chunk_bytes)):
            resumable.receive_chunk(session, index=index, payload=chunk, storage_backend=storage)
...
class TestResuming:
    def test_the_session_says_which_chunks_are_missing(self, db, project, storage) -> None:
    ...
    def test_an_interrupted_upload_finishes_by_sending_the_rest(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
        chunks = _chunks(PAYLOAD, session.chunk_bytes)
    ...
    def test_chunks_may_arrive_out_of_order(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
        chunks = _chunks(PAYLOAD, session.chunk_bytes)
    ...
    def test_resending_a_chunk_replaces_it_rather_than_appending(self, db, project, storage) -> None:
...
class TestRefusals:
    def test_a_truncated_chunk_is_caught_by_the_checksum(self, db, project, storage) -> None:
    ...
    def test_a_short_middle_chunk_is_refused_on_arrival(self, db, project, storage) -> None:
    ...
    def test_a_chunk_past_the_end_is_refused(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
    ...
    def test_an_empty_chunk_is_refused(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
    ...
    def test_a_declared_size_of_zero_is_refused(self, db, project) -> None:
        with pytest.raises(BadRequestError, match="positive"):
            resumable.SESSIONS.create(
                db, project_id=project, file_name="x.csv", content_type="", total_bytes=0
            )


class TestIsolation:
    def test_another_project_cannot_reach_this_session(self, db, project, storage) -> None:
        """An upload id is otherwise a guess away from another project's file."""
        session = _session(db, project, PAYLOAD)
        with pytest.raises(NotFoundError):
            resumable.SESSIONS.get(db, session.id, project_id=uuid.uuid4())
        assert resumable.SESSIONS.get(db, session.id, project_id=project).id == session.id

    def test_an_unknown_upload_id_is_not_found(self, db, project) -> None:
        with pytest.raises(NotFoundError):
            resumable.SESSIONS.get(db, uuid.uuid4(), project_id=project)


class TestBounds:
    def test_a_huge_file_gets_larger_chunks_rather_than_a_refusal(self, db, project) -> None:
        ...
        session = resumable.SESSIONS.create(
            db, project_id=project, file_name="huge.csv", content_type="", total_bytes=huge
        )
        assert session.expected_chunks <= resumable.MAX_CHUNKS
        assert session.chunk_bytes > resumable.CHUNK_BYTES

    def test_a_two_gigabyte_file_is_a_few_hundred_requests(self, db, project) -> None:
...
class TestDurability:
    """The point of the durable store: a gateway restart does not orphan an
    upload. Re-reading the session from the database is exactly what a fresh
    process does when the client asks 'which chunks arrived?' after a restart."""

    def test_received_chunks_survive_a_restart(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
        chunks = _chunks(PAYLOAD, session.chunk_bytes)
        for index in range(2):
            resumable.receive_chunk(
        ...
        # A different gateway process loads the same session from the database.
        reloaded = resumable.SESSIONS.get(db, session.id, project_id=project)
        assert sorted(reloaded.received) == [0, 1]
        assert 0 not in reloaded.missing and 1 not in reloaded.missing

        # It finishes by sending the rest -- the parts on disk are still valid.
        for index in reloaded.missing[:]:
            resumable.receive_chunk(
                reloaded, index=index, payload=chunks[index], storage_backend=storage
            )
        assert resumable.assemble(reloaded, storage_backend=storage) == PAYLOAD

    def test_dropping_a_session_removes_the_row(self, db, project, storage) -> None:
        session = _session(db, project, PAYLOAD)
        resumable.SESSIONS.drop(db, session.id)
        with pytest.raises(NotFoundError):
            resumable.SESSIONS.get(db, session.id, project_id=project)
