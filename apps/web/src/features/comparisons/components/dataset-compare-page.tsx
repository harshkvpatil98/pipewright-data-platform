    ...
    >
      ...
      >
        <div className="flex flex-col gap-4 md:flex-row md:flex-wrap md:items-end">
          <label className="flex min-w-[200px] flex-col gap-1 text-xs uppercase tracking-[0.16em] text-slate-500">
            Test type
            <select
          ...
          <label className="flex min-w-[200px] flex-1 flex-col gap-1 text-xs uppercase tracking-[0.16em] text-slate-500">
            Column name
            <input
              className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-slate-100"
              value={columnName}
        ...
        {testResult ? (
          <div className="mt-5 space-y-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 text-sm text-slate-200">
            <div className="flex flex-wrap gap-2 text-xs uppercase tracking-[0.16em] text-slate-500">
              ...
              <span>·</span>
              <span className="font-mono text-slate-300">{testResult.column_name}</span>
            </div>
            <dl className="grid gap-2 md:grid-cols-2">
              ...
              <div>
                <dt className="text-xs text-slate-500">Sample sizes</dt>
                <dd>
                  {testResult.left_dataset.sample_size} (left) · {testResult.right_dataset.sample_size} (right)
                </dd>
              </div>
            </dl>
            <p className="text-slate-300">{testResult.interpretation}</p>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <label className="flex flex-col gap-1 text-xs uppercase tracking-[0.16em] text-slate-500">
            ...
            <textarea
              className="min-h-[88px] rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-slate-100"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionPanel title="Schema changes" description="Diff of schema_json column names and inferred types.">
          <dl className="space-y-3 text-sm">
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Added columns</dt>
              <dd className="mt-1 text-slate-200">
                {schema_delta.added_columns.length ? schema_delta.added_columns.join(", ") : "—"}
              </dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Removed columns</dt>
              <dd className="mt-1 text-slate-200">
                {schema_delta.removed_columns.length ? schema_delta.removed_columns.join(", ") : "—"}
              </dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Inferred type changes</dt>
              <dd className="mt-1 space-y-1">
                {schema_delta.changed_type_columns.length === 0 ? (
                  <span className="text-slate-400">—</span>
                ) : (
                  schema_delta.changed_type_columns.map((c) => (
                    <div key={c.column_name} className="font-mono text-xs text-slate-300">
        ...
        <SectionPanel title="Profile / quality delta" description="From profile_json when present.">
          <dl className="grid gap-3 text-sm md:grid-cols-2">
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Duplicate rows (left)</dt>
              <dd className="mt-1 text-slate-200">{formatNumber(profile_delta.duplicate_row_count_before)}</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Duplicate rows (right)</dt>
              <dd className="mt-1 text-slate-200">{formatNumber(profile_delta.duplicate_row_count_after)}</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Completeness (left)</dt>
              <dd className="mt-1 text-slate-200">
            ...
            <div>
              <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Completeness (right)</dt>
              <dd className="mt-1 text-slate-200">
      ...
      <SectionPanel title="Lineage context" description="Parent/child and pipeline links when detectable from dataset rows.">
        <dl className="grid gap-3 text-sm md:grid-cols-2">
          <div>
            <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Parent/child relationship</dt>
            <dd className="mt-1 text-slate-200">{lineage_context.related_by_parent_child ? "Yes" : "No"}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Parent dataset id</dt>
          ...
          <div>
            <dt className="text-xs uppercase tracking-[0.18em] text-slate-500">Related run id</dt>
            <dd className="mt-1 font-mono text-xs text-slate-300">{lineage_context.related_run_id ?? "—"}</dd>
          </div>
        </dl>
      </SectionPanel>
    </AppShell>
